  ...
  if (choosenQueue) {
    ...
    if (choosenQueue.options.length === 0) {
      ...
      if (queue.outOfHoursMessage !== null && queue.outOfHoursMessage !== "" && !isNil(schedule)) {
        ...
        // Verifica se o horário atual está fora do horário de atendimento ou dentro da pausa
        const isOutsideWorkingHours = now.isBefore(startTime) || now.isAfter(endTime ) ;
  ...
  try {
    ...
    try {
      if (!msg.key.fromMe && scheduleType) {
        ...
        if (
          ...
          (currentSchedule.inActivity === false || isOutsideWorkingHours(currentSchedule))
        ) {
        ...
        // Função para verificar se o horário atual está fora do expediente ou durante a pausa
        function isOutsideWorkingHours(schedule) {

          const now = moment();
          const startTime = moment(schedule.startTime, "HH:mm");
          const endTime = moment(schedule.endTime, "HH:mm");

          const hasPause = schedule.startPause && schedule.endPause;
          const startPause = hasPause ? moment(schedule.startPause, "HH:mm") : null;
          const endPause = hasPause ? moment(schedule.endPause, "HH:mm") : null;
          
          if (now.isBefore(startTime) || now.isAfter(endTime) || now.isBetween(startPause, endPause)) {
            return true;
        }
        if (hasPause && now.isBetween(startPause, endPause)) {
          return true;
        }

      }
        console.log(isOutsideWorkingHours)
        console.log('Mensagem:', bodyMessage);
        ...
        if (scheduleType.value === "queue" && ticket.queueId !== null) {
          ...
          if (
            ...
            const startTime = moment(schedule.startTime, "HH:mm");
            const endTime = moment(schedule.endTime, "HH:mm");
            const startPause = moment(schedule.startPause, "HH:mm");
            const endPause = moment(schedule.endPause, "HH:mm");
    ...
    try {
      ...
      if (!msg.key.fromMe && scheduleType && ticket.queueId !== null) {
        ...
        const weekday = now.format("dddd").toLowerCase();
        let schedule = null;
        //segunda tentativa // Aqui é onde é feito os horários de atendimento
        if (Array.isArray(schedules) && schedules.length > 0) {
          schedule = schedules.find(
            s =>
              s.weekdayEn === weekday &&
